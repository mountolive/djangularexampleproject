...
function login(email,password){return $http.post('/api/v1/auth/login/',{email:email,password:password}).then(loginSuccessFn,loginErrorFn);function loginSuccessFn(response){Authentication.setAuthenticatedAccount(response.data);window.location='/';}
...
function login(){Authentication.login(vm.email,vm.password);}}})();;(function(){'use strict';angular.module('mountolive.layout',['mountolive.layout.controllers']);angular.module('mountolive.layout.controllers',[]);})();;(function(){'use strict';angular.module('mountolive.layout.controllers').controller('NavbarController',NavbarController);NavbarController.$inject=['$scope','Authentication'];function NavbarController($scope,Authentication){var vm=this;vm.logout=logout;function logout(){Authentication.logout();}}})();