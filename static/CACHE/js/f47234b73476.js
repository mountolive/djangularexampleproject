...
function($locationProvider){$locationProvider.html5Mode(true);$locationProvider.hasPrefix('!');}})();;(function(){'use strict';angular.module('mountolive.routes').config(config);config.$inject=['$routeProvider'];function config($routeProvider){$routeProvider.when('/register',{controller:'RegisterController',controllerAs:'vm',template:require('./register.html')}).when('/login',{controller:'LoginController',controllerAs:'vm',template:require('./login.html')}).otherwise('/');}})();;(function(){'use strict';angular.module('mountolive.authentication',['mountolive.authentication.controllers','mountolive.authentication.services']);angular.module('mountolive.authentication.controllers',[]);angular.module('mountolive.authentication.services',['ngCookies']);})();;(function(){'use strict';angular.module('mountolive.authentication.services').factory('Authentication',Authentication);Authentication.$inject=['$cookies','$http'];function Authentication($cookies,$http){var Authentication={login:login
register:register};return Authentication;function register(email,password,username){return $http.post('/api/v1/accounts/',{getAuthenticatedAccount:getAuthenticatedAccount,unauthenticate:unauthenticate,setAuthenticatedAccount:setAuthenticatedAccount,isAuthenticated:isAuthenticated,username:username,password:password,email:email}).then(registerSuccessFn,registerErrorFn);function registerSuccessFn(data,status,headers,config){Authentication.login(email,password);}
